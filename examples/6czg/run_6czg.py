#!/usr/bin/env python3
"""Four-Way Crystallographic Refinement Comparison on De Novo Designed Beta-Barrel (6CZG).

Disentangles the target likelihood formulation from Hessian curvature preconditioning
via a full 2x2 factorial comparison on a de novo designed protein (David Baker & Banumathi Sankaran):
  ...
  1. Classic F-based likelihood (ml_f, unpreconditioned):
     ...
     - PyTorch L-BFGS joint estimation of sigma_A across resolution bins
  ...
  3. Intensity-based likelihood (ml_i, unpreconditioned):
     ...
     - PyTorch optimization of sigma_A and nu
...
Usage::
    ...
    # Run full 4-way comparison (default 3 steps per mode, 2.2 Å resolution limit)
    python examples/6czg/run_6czg.py --steps 3 --d-min 2.2

    # Run a specific mode only
    python examples/6czg/run_6czg.py --mode precond_intensity --steps 5
"""
...
def prepare_data(force_download: bool = False) -> tuple[Path, Path]:
    print("\n--- 1. Fetching 6CZG Data from RCSB PDB ---")
    if force_download:
        if PDB_FILE.exists():
            PDB_FILE.unlink()
        if CIF_FILE.exists():
    ...
    download_file("https://files.rcsb.org/download/6czg.pdb", PDB_FILE)
    download_file("https://files.rcsb.org/download/6czg-sf.cif", CIF_FILE)
    ...
    if not MTZ_FILE.exists() or MTZ_FILE.stat().st_size == 0:
        ...
        status = None
        for a in arrays:
            lbls = " ".join(a.info().labels if a.info() else [])
            if a.is_xray_intensity_array():
                iobs = a
            elif "status" in lbls.lower() and a.is_string_array():
                status = a
        ...
        if status is not None:
            ...
            iobs_common, status_common = iobs.common_sets(status)
            s_data = status_common.data()
            r_free_data = (s_data == "f") | (s_data == "F")
            r_free = iobs_common.customized_copy(data=r_free_data, sigmas=None)
            iobs = iobs_common
        else:
...
def run_single_refinement(
    ...
    print(f" Target: {target} | Preconditioner: {use_preconditioner} | Steps: {steps} | Device: {device}")
    print("=" * 70)
...
def print_comparison_table(results: List[Dict[str, Any]]) -> None:
    print("\n" + "=" * 115)
    print(" CRYSTALLOGRAPHIC REFINEMENT COMPARISON TABLE (6CZG)")
    print("=" * 115)
    ...
    rows = [
        ...
        ("Initial R_int (free)", [f"{r['init_summary']['r_int_free']*100:.2f}%" for r in results]),
        ("Final R_int (free)", [f"{r['final_summary']['r_int_free']*100:.2f}%" for r in results]),
...
def plot_comparison(results: List[Dict[str, Any]], out_png: Path) -> None:
    ...
    fig.suptitle("De Novo Beta-Barrel (6CZG): Refinement Comparison", fontsize=15, fontweight="bold")
...
def main(args: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Four-way refinement test on 6CZG: classic F vs precond F vs intensity vs precond intensity.",
    )
    parser.add_argument(
        "--mode",
        default="all",
    ...
    parser.add_argument("--output-dir", default=str(DIR_6CZG), help="Output directory for results")
    ...
    parser.add_argument("--force-download", action="store_true", help="Force re-download and re-conversion of 6CZG files")
    ...
    if opts.mode in ("all", "f_based"):
        modes_to_run.append({
            ...
            "prefix": str(out_dir / "6czg_f_unprecond"),
        })
    if opts.mode in ("all", "precond_f"):
        modes_to_run.append({
            ...
            "prefix": str(out_dir / "6czg_f_precond"),
        })
    if opts.mode in ("all", "intensity"):
        modes_to_run.append({
            ...
            "target": "intensity",
            "use_preconditioner": False,
            "prefix": str(out_dir / "6czg_i_unprecond"),
        })
    if opts.mode in ("all", "precond_intensity"):
        modes_to_run.append({
            ...
            "target": "intensity",
            "use_preconditioner": True,
            "prefix": str(out_dir / "6czg_i_precond"),
        })
    ...
    if not opts.no_plot and len(results) > 1:
        plot_file = out_dir / "four_way_comparison.png"
