#!/usr/bin/env python3
"""Direct Re-Refinement of Deposited 1EE2 Structure (No Shaking / Perturbation).

Performs clean, unperturbed re-refinement of the deposited 1EE2 structure (1.54 Å) using:
  1. Amplitude maximum likelihood target (ml_f, F)
  2. Intensity log-likelihood target (ml_i, I) with Student-t noise model (nu ~ 7)

Key Features:
  - Intact stereochemical geometry scale (w_geom = 1.0, preserving physical units).
  - Physical unit weighting: w_xray = xray_scale * N_work, w_geom = 1.0.
...
PDB_REF = DIR_1EE2 / "1ee2.pdb"
MTZ_FILE = DIR_1EE2 / "1ee2.mtz"


def main():
    parser = argparse.ArgumentParser(description="Clean re-refinement of deposited 1EE2 (no shaking)")
    parser.add_argument("--pdb", default=str(PDB_REF), help="Deposited 1EE2 PDB")
    parser.add_argument("--mtz", default=str(MTZ_FILE), help="Experimental 1EE2 MTZ")
    parser.add_argument("--d-min", type=float, default=1.54, help="Resolution cutoff (default: 1.54 Å)")
    ...
    parser.add_argument("--out-dir", default=str(DIR_1EE2 / "rerefined_no_shake"), help="Output directory")
    ...
    print("=" * 85)
    print(" 1EE2 UNPERTURBED DEPOSITED STRUCTURE RE-REFINEMENT")
    print(f" PDB: {pdb_path.name} | MTZ: {mtz_path.name} | d_min: {args.d_min} Å")
    ...
    out_f = out_dir / "1ee2_amplitude_rerefined.pdb"
    out_i = out_dir / "1ee2_intensity_rerefined.pdb"
    ...
    if not out_f.exists() or args.force_refine:
        ...
        m_f = from_files(str(out_f), str(mtz_path), d_min=args.d_min, device="cpu", use_bulk_solvent=True)
        ...
        b_dep_arr = [sc.u_iso * 8.0 * np.pi**2 for sc in from_files(str(pdb_path), str(mtz_path), d_min=args.d_min, device="cpu").xray_structure.scatterers()]
    ...
    if not out_i.exists() or args.force_refine:
        ...
        m_i = from_files(str(out_i), str(mtz_path), d_min=args.d_min, device="cpu", use_bulk_solvent=True)
    ...
    ref_model = from_files(str(pdb_path), str(mtz_path), d_min=args.d_min, device="cpu", use_bulk_solvent=True)
    ...
    comparisons = [
        ("rerefined_f_vs_deposited", "deposited_1ee2", str(pdb_path), "rerefined_f", str(out_f)),
        ("rerefined_i_vs_deposited", "deposited_1ee2", str(pdb_path), "rerefined_i", str(out_i)),
    ...
    for comp_id, name_a, path_a, name_b, path_b in comparisons:
        ...
        mA = from_files(path_a, str(mtz_path), d_min=args.d_min, device="cpu", use_bulk_solvent=True)
        ...
        mB = from_files(path_b, str(mtz_path), d_min=args.d_min, device="cpu", use_bulk_solvent=True)
    ...
    # Summary table
    print("\n" + "=" * 125)
    print(" SUMMARY OF UNPERTURBED 1EE2 RE-REFINEMENT")
    print("=" * 125)
    ...
    m_dep_eval = from_files(str(pdb_path), str(mtz_path), d_min=args.d_min, device="cpu", use_bulk_solvent=True, target="intensity")
    ...
    m_f_eval = from_files(str(out_f), str(mtz_path), d_min=args.d_min, device="cpu", use_bulk_solvent=True, target="intensity")
    ...
    m_i_eval = from_files(str(out_i), str(mtz_path), d_min=args.d_min, device="cpu", use_bulk_solvent=True, target="intensity")
    ...
    print(f"{'Deposited (1ee2.pdb)':<22} | {s_dep['r_work']*100:5.2f}% | {s_dep['r_free']*100:5.2f}% | {s_dep['cc_free_i']:8.4f}   | {np.mean(b_dep):5.2f}Å² | {'1.0000':<7} | {s_dep['target_work']:9.4f} | {nll_test_dep:9.4f} | {adp_e_dep:<7.1f} | {gst_dep['bond_rms']:7.4f} | {gst_dep['angle_rms']:6.2f}° | {gst_dep['planarity_rms']:7.4f} | {'[ACCEPTABLE]' if gst_dep['is_acceptable'] else '[OUT]'}")
