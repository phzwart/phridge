...
Usage::
    ...
    PYTHONPATH=src python -m phridge.contrib.nufft_sf.benchmark \\
        --cases synthetic --d-min 2.5,2.0 --device cpu --repeats 5
"""
...
def run_case(
    ...
    xs = None
    if cctbx_ok and case in ("1ee2", "6czg", "synthetic"):
        try:
    ...
    ref_direct: Optional[np.ndarray] = None
    if xs is not None:
        try:
            ref_direct = np.array(xs.structure_factors(d_min=d_min, algorithm="direct").f_calc().data())
        except Exception:
            ref_direct = None
    ...
    if xs is not None:
        ...
        def _direct():
            return np.array(xs.structure_factors(d_min=d_min, algorithm="direct").f_calc().data())

        f_fft, t_fft = _median(_fft, "cpu", repeats)
        f_dir, t_dir = _median(_direct, "cpu", repeats)
        r_fft = _r_factor(f_fft, f_dir) if f_dir is not None else float("nan")
        rows.append(
            Row("cctbx_fft", case, d_min, "cpu", 0, 0, r_fft, t_fft, float("nan"), float("nan"), model.n_scatterers, len(hkl))
        )
        rows.append(
            Row("cctbx_direct", case, d_min, "cpu", 0, 0, 0.0, t_dir, float("nan"), float("nan"), model.n_scatterers, len(hkl))
        )
    return rows
...
def format_table(rows: list[Row]) -> str:
    header = (
        ...
        "R(F) vs cctbx direct | t(F) | t(F+grad) | peak mem (MB) |"
    )
    sep = "|---|---|---|---|---|---|---|---|---|---|---|---|"
    lines = [header, sep]
    for r in rows:
        rf = "n/a" if not np.isfinite(r.r_vs_direct) else f"{r.r_vs_direct:.3e}"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p.add_argument("--cases", default="synthetic", help="comma list: 1ee2,6czg,synthetic")
    p.add_argument("--d-min", default="2.5,2.0,1.5")
    p.add_argument("--device", default="cpu")
    p.add_argument("--repeats", type=int, default=5)
    p.add_argument("--n-synthetic", type=int, default=20000)
    ...
    for case in cases:
        for d_min in dmins:
            print(f"=== {case} d_min={d_min} device={args.device} ===")
            rows.extend(run_case(case, d_min, args.device, args.repeats, args.n_synthetic))
