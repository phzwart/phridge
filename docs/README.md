...
- [Redis store, remote jobs, and local fakes](redis.md)
...
Science types live in `schema/cctbx*.yaml`. Redis is mandatory and the
only shared process boundary: the Phenix client never imports torch; the
worker never imports cctbx. See [redis.md](redis.md).
